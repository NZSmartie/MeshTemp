...
static void set_symbol_impl(struct device *dev, u8_t symbol, u8_t state)
{
    ...
    if (symbol >= SYMBOLS)
    {
        LOG_ERR("Symbol %d is out of range 0-%d", symbol, SYMBOLS - 1);
        return;
    }

    if (state >= 10)
    {
        LOG_ERR("Cannot display %d for symbol %d", state, symbol);
        return;
    }

    const struct bu9795_segment *mapping = &config->segment_mapping->symbols[symbol];
    for(int i = 0; i < BU9795_SEG_REGISTER_SIZE; i++)
    {
        data->data[i] &= mapping->mask[i];
        data->data[i] |= mapping->digits[state][i];
    }
    ...
    .symbols = {
        ...
        },
        {
            ...
            .digits = {
                ...
                {0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00},
                {0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x03, 0x00, 0x00, 0x00, 0x20, 0x00, 0x80, 0x00},
                {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
